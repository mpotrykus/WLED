...
class DeskMod : public Usermod {
  ...
  private:
    ...
    uint16_t s1DupStart = 15;
    uint16_t s1DupEnd = 15;
  ...
  public:
    ...
    void handleOverlayDraw()
    {
      ...
      uint16_t currentMap = ledMaps["ledmap" + mode];
    ...
    void duplicateLeds(uint16_t ledStart, uint16_t ledEnd)
    {
      // TODO: Use predefined array, rather than math.
      uint16_t perc = (s1End - s1Start)/(s1DupStart - s1DupEnd);
      // 10 - 5 (5) / 12 - 8 (4) = 1.25

      for (uint16_t led = ledStart; led <= ledEnd; led++) 
      {
        uint16_t copyIndex = floor_t(perc * led);
        uint32_t color = strip.getPixelColor(copyIndex);
